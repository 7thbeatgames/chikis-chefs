...
function put_basketbeats_in_pattern(vol,pattern_num)
	...
	for i=1,#arr_basket_beats do
		-- x,x,bit1,bit2,bit3
		--set_vol(pattern_num, arr_basket_beats[i], true,false,false)
	end
	--cstore()
end
...
function set_vol(sfx,time,bit1,bit2,bit3)
  ...
  note = peek(addr+1)
  if (bit1) then note = note | 0b100 
  else
   note = note & 0b1111111111111011.1111111111111111
  end
  if (bit2) then note = note | 0b101
  else
   note = note & 0b1111111111111101.1111111111111111
  end
  if (bit3) then note = note | 0b110
    else
   note = note & 0b1111111111111110.1111111111111111
  end
  poke(addr+1,note)
end
